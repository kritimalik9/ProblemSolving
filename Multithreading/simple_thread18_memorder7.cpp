...
// memory_order_release - memory_order_acquire guarantee order but not data variable caching issue
...
struct thread_info
{
    void writer_thread()
    {
        ...
        has_data.store(true, std::memory_order_release); // release memory lock here, data assignment will be happen before.
    }
    void reader_thread()
    {
