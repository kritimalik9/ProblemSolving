...
struct new_thread_class
{
    ...
    void print_from_new_thread(int id, int& ref_val, int* ref_val_dynamic, std::function<int(int,int)> f)
    {
        ...
        *(ref_val_dynamic) = (*ref_val_dynamic) - 20;
        ...
        std::cout << "Inside new_thread_class, thread id:  " << std::hex << std::this_thread::get_id()
            ...
            << ", Internal id: " << id
            << ", ref_val: " << ref_val
            << ", ref_val-f: " << f(ref_val, 8)
            << ", ref_val_dynamic: " << *(ref_val_dynamic)
            << ", ref_val_dynamic-f: " << f(*(ref_val_dynamic), 24) << std::endl;
    }
    ...
    new_thread_class(int thr1_id, int thr3_id, int& ref_val, int* ref_val_dynamic, std::function<int(int,int)>& f)
...
int main(int argc, const char* argv[])
{
    ...
    std::function<int(int, int)> f = std::bind(&main_thread_class::add, &m, std::placeholders::_2, 38, std::placeholders::_1);
